...
async function main() {  
  ...
  try {
    ...
    if (XMLValidator.validate(XMLdata)) {
      ...
      sampleXmlData = builder.build(jsonObj);
      fList.forEach(async (file) => {
        if (file.name === 'public/HubbleHomes_zillow_feed.xml') {
          url = file.url;          
        }
      });
      await files.write(`public/${filename}`, sampleXmlData);
    }
    
    logger.info('list = ', fList);
    const response = {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: `XML data is written to ${url}`,
    };
...
async function fetchWorkbook(sheetNames) {
  ...
  const url = `https://main--hubblehomes-com--aemsites.aem.live/data/hubblehomes.json?${sheets.map((sheetName) => `sheet=${sheetName}`).join('&')}`;
...
async function addInventoryData(model, community) {
  ...
  for (const inventoryhome of inventoryData.data) {
    if (inventoryhome.community.toLowerCase() === community.toLowerCase() && inventoryhome['model name'].toLowerCase() === model.toLowerCase()) {
      ...
      const pageURL = "https://main--hubblehomes-com--aemsites.aem.live" + inventoryhome.path + ".plain.html";
      ...
      allImageTags.forEach((img, index) => {
        ...
        imagexml += `<SpecInteriorImage SequencePosition="${index + 1}" Title="" 
          ...
          ${(`https://main--hubblehomes-com--aemsites.aem.live${inventoryhome.path}${imgsrc[0]}`)}</SpecInteriorImage>`;
        if (index === 0) {
          elevations = `<SpecElevationImage SequencePosition="1" Title="${model}" Caption="" ReferenceType="URL">${(`https://main--hubblehomes-com--aemsites.aem.live${inventoryhome.path}${imgsrc[0]}`)}</SpecElevationImage>`;
        }
      });
      imagexml += elevations;
      ...
      FloorPlanDiv.forEach((div, index) => {
        ...
        floorPlanImages += `<SpecFloorPlanImage SequencePosition="${index}" Caption="${label}" Title="${label}" ReferenceType="URL"> 
          ${`https://main--hubblehomes-com--aemsites.aem.live${inventoryhome.path}${imgsrc[0]}`}</SpecFloorPlanImage>` 
      });
      let matterportURL;
...
async function addImagesDescription(path, modelName) {
  try {
  const pageURL = "https://main--hubblehomes-com--aemsites.aem.live" + path + ".plain.html";
  const response = await fetch(pageURL);
  const pageText = await response.text();
  const dom = new JSDOM(pageText);
  const body = dom.window.document.getElementsByTagName("body")[0];
  ...
  allImageTags.forEach((img, index) => {
    ...
    ${(`https://main--hubblehomes-com--aemsites.aem.live${path}${imgsrc[0]}`)}</InteriorImage>`;
  });
  ...
  allElevationdiv.forEach((div, index) => {
    ...
    ${(`https://main--hubblehomes-com--aemsites.aem.live${path}${imgsrc[0]}`)}</ElevationImage>`
  });
  let floorPlanImages = '';
  const FloorPlanDiv = body.querySelectorAll(".floorplan > div");
  FloorPlanDiv.forEach((div, index) => {
    ...
    ${`https://main--hubblehomes-com--aemsites.aem.live${path}${imgsrc[0]}`}</FloorPlanImage>`
  });
  let matterportURL;
  let contradovipURL;
  const linksblock = body.querySelectorAll(".links > div > div > a");
  linksblock.forEach((a) => {
    if (a.href.includes('pdf')) {
      contradovipURL = `https://main--hubblehomes-com--aemsites.aem.live${a.href}`;
...
async function addCarouselImages(path) {
  ...
  allImageTags.forEach((img) => {
    const imgsrc = (img.src.substring(1)).split('?');
    imagehref.push(`https://main--hubblehomes-com--aemsites.aem.live${path}${imgsrc[0]}`);
  });
...
async function addSubDivison(subdivision, salesoffice, carouselImages) {
  ...
  const pageURL = "https://main--hubblehomes-com--aemsites.aem.live" + subdivision.path + ".plain.html";
  const response = await fetch(pageURL);
  const pageText = await response.text();
  const dom = new JSDOM(pageText);
  const body = dom.window.document.getElementsByTagName("body")[0];
